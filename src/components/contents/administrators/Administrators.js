...
export default function Administrators() {
  const dataAdmin = async () => {
    ...
    --------------------------------------*/
    const getAdmin = await getData();

    const dataSet = [];

    getAdmin.data.forEach((item, index) => {
      dataSet[index] = [index + 1, item.user, [item._id, item.user]];
    });
        ...
        <div className="container-fluid">
          <div className="row mb-2">
            <div className="col-sm-6">
              <h1 className="m-0 text-dark">Administrators</h1>
            </div>
          </div>
        </div>
      </div>
...
--------------------------------------*/
const getData = () => {
  const url = `${urlAPI}/mostrar-administradores`;
